                                        ...
                                        for xc0px in xa5:
                                            ...
                                            """ if cnn0+cnnk!=0:
                                                ...
                                                if xx1==1 and F3[x2]<-0.00001 and abs(F2[x2])<0.00001:
                                                    continue
                                            if cnn0p+cnnkp!=0:
                                                if xx2==1 and F1[x2]>0.00001 and F3[x2]>=-0.00001:#dxcp增#1MT聚合 解聚
                                                    continue
    ...
    for cx0 in range(len(ji[0])):
        ...
        for cx1 in range(len(ji[1])):
            ...
            for cx2 in range(len(ji[2])):
                ...
                #b=np.array([round(-F0,10),round(-F7,10),round(-F5,10),round(-F6,10),round(b4,10),round(b5,10),round(b6,10),round(b7,10),round(b8,10),round(b9,10),round(b10,10),round(b11,10),round(b12,10),round(b13,10),round(b14,10),round(b15,10)])
                #b=np.array([round(-F0,6),round(-F7,6),round(-F5,6),round(-F6,6),round(b4,6),round(b5,6),round(b6,6),round(b7,6),round(b8,6),round(b9,6),round(b10,6),round(b11,6),round(b12,6),round(b13,6),round(b14,6),round(b15,6)])
                #b=np.array([round(-F0,4),round(-F7,4),round(-F5,4),round(-F6,4),round(b4,4),round(b5,4),round(b6,4),round(b7,4),round(b8,4),round(b9,4),round(b10,4),round(b11,4),round(b12,4),round(b13,4),round(b14,4),round(b15,4)])
                ...
                #print(de,'xx',dzkpx[2][xx3][ta5]-dz0px[2][xx3][ta6])
                #if cxx[2]==1:
                #    print(jix)
                chezq=0
...
def gaussian_elimination(A, b):
    ...
    for i in range(n - 1):
        ...
        for j in range(n-1,i ,-1 ):
            ...
            b[j] -= multiplier * b[i]
            """if j==n-1 and abs(A[j][i])>=0.000000001:
                for kj in range(j-1,i,-1):
                    print(kj)
                    if abs(A[kj][i])>=0.00001 and abs(b[kj])<=0.00001:
                        print(A[kj][i])
                        multiplier = A[j][i] / A[kj][i]
                        print(multiplier)
                        print(multiplier * A[kj][i:])
                        A[j][i:] -= multiplier * A[kj][i:]
                        b[j] -= multiplier * b[kj]
                        break
                    if kj==i+1:
                        multiplier = A[j][i] / A[i][i]
...
"""for xn in range(18):
    kinam[4].append(1)
    kinbm[4].append(1)
    cx0=l2l[1]+round(lover/2/d*random.uniform(0,1))*d
    numa[4].append([cx0,cx0])
for xn in range(18):
    kinam[5].append(1)
    kinbm[5].append(1)
    cx0=l3r[1]-round(lover/2/d*random.uniform(0,1))*d
...
"""for xn in range(18):
    kinam1[4].append(1)
    kinbm1[4].append(1)
    cx0=l1l[1]+round(lover/2/d*random.uniform(0,1))*d
    numa1[4].append([cx0,cx0])
for xn in range(18):
    kinam1[5].append(1)
    kinbm1[5].append(1)
    cx0=l4r[1]-round(lover/2/d*random.uniform(0,1))*d
...
0.0003/0.003 Kp0.1/10/0.03 5-4-0.5 Fp1.85 30 dif82 1nM 3d/2 0.0003 0.1


# In[18]:


print(a,'lxian=',lxian,'Xpole=',Xpole,'Xpo=',Xpo,'\n','\n','l1l=',l1l,'\n','l1r=',l1r,'\n','l2l=',l2l,'\n','l2r=',l2r,'\n',
      'l3l=',l3l,'\n','l3r=',l3r,'\n', 'l4l=',l4l,'\n','l4r=',l4r,'\n', 'kina=',kina,'\n','kinb=',kinb,'\n','kinap=',kinap,'\n','kinbp=',kinbp,'\n','kinapx=',kinapx,'\n','kinbpx=',kinbpx,'\n',
      'motor =',motor,'\n', 'motorp =',motorp,'\n','motorpx =',motorpx,'\n' ,'numa =',numa,'\n' ,'kinam =',kinam,'\n','kinbm =',kinbm,'\n' ,'numa1 =',numa1,'\n' ,'kinam1 =',kinam1,'\n','kinbm1 =',kinbm1,'\n'
      'motormc1 =',motormc1,'\n', 'motormc2 =',motormc2,'\n','motormc3 =',motormc3,'\n','motormc4 =',motormc4,'\n','Xkin =',Xkin,'\n','Xkin1 =',Xkin1,'\n')
...
ckF,de0,de1,de2,de3,de4,de5,de6,de7=checkF()#1,0,[0,0,0],[0, 0, 0],[-6, 0,6],[0,0, 0],0,0,0#checkF()#
for x2 in range(pair):
    for xn in range(nu[x2]):
        if kina[x2][xn]==1:
            motor[x2][xn][0]+=de1[x2]
